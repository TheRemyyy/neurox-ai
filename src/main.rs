    ...
    loop {
        ...
        if input.starts_with('/') {
            match input {
                ...
                "/help" => {
                    println!("{}PŘÍKAZY:{}", COLOR_CYAN, COLOR_RESET);
                    println!("  /train <soubor>  - Trénuj (.txt=Skip-gram, .json=supervised)");
                    println!("  /learn <text>    - Okamžité učení z textu");
                    println!("  /vocab           - Zobraz naučenou slovní zásobu");
                    println!("  /sleep           - Spuštění konsolidace paměti");
                    println!("  /shock           - Injekce norepinefrinu (arousal)");
                    println!("  /dopamine        - Injekce dopaminu (reward)");
                    println!("  /debug           - Debug informace");
                    println!("  exit             - Ukončení");
                }
                "/vocab" => {
                    let vocab_size = brain.language.ventral.embeddings.idx_to_word.len();
                    println!("{}SLOVNÍ ZÁSOBA ({} slov):{}", COLOR_CYAN, vocab_size, COLOR_RESET);
                    for (i, word) in brain.language.ventral.embeddings.idx_to_word.iter().take(50).enumerate() {
                        print!("{} ", word);
                        if (i + 1) % 10 == 0 { println!(); }
                    }
                    if vocab_size > 50 {
                        println!("\n  ... a dalších {} slov", vocab_size - 50);
                    }
                    println!();
                }
                _ if input.starts_with("/train ") => {
                    ...
                    println!();
                    println!("{}{} >{} Loading: {} ({})", 
                        BOLD, "TRAIN", COLOR_RESET, file_path, if is_json { "JSON supervised" } else { "TEXT skip-gram" });
                    
                    if is_json {
                        // JSON supervised learning via BrainLoader
                        match neurox_ai::brain::loader::BrainLoader::load_json_training(&mut brain, file_path) {
                            Ok(_) => {
                                println!("{}✓ Naučeno!{} {} slov v lexiconu", 
                                    COLOR_GREEN, COLOR_RESET,
                                    brain.lexicon.words.len());
                                mode = "INTERACTIVE".to_string();
                            }
                            Err(e) => {
                                println!("{}CHYBA: Nelze načíst JSON: {}{}", COLOR_RED, e, COLOR_RESET);
                            }
                        }
                    } else {
                        // Text file via BrainLoader
                        match neurox_ai::brain::loader::BrainLoader::load_text_training(&mut brain, file_path, 3) {
                            Ok(_) => {
                                println!("\n{}>>> TRÉNINK DOKONČEN! Slovník: {} slov{}", 
                                    COLOR_GREEN, brain.language.ventral.embeddings.idx_to_word.len(), COLOR_RESET);
                                mode = "INTERACTIVE".to_string();
                            }
                            Err(e) => {
                                println!("{}CHYBA: Nelze načíst soubor: {}{}", COLOR_RED, e, COLOR_RESET);
                            }
                        }
                    }
                }
                _ => println!("{}Neznámý příkaz. Napiš /help pro seznam příkazů.{}", COLOR_RED, COLOR_RESET),
            }
            continue;
        }
