...
impl CerebellarHemisphere {
    ...
    fn build_connectivity_with_seed(
        ...
        let gr_to_pk: Vec<Vec<(usize, f32)>> = (0..8)
            .map(|_| (0..512).map(|_| (rng.gen_range(0..4096), 0.01)).collect())
            .collect();
    ...
    pub fn stats(&self) -> CerebellarStats {
        let active_granule = self
            .granule_cells
            .iter()
            .filter(|gc| self.timestep - gc.last_spike < 10)
            .count();

        let active_purkinje = self
            .purkinje_cells
            .iter()
...
mod tests {
    ...
    fn test_cerebellar_learning() {
        ...
        let initial_weights: Vec<f32> = hemisphere.purkinje_cells[0].parallel_fiber_weights.clone();
        ...
        let final_weights_ltp = hemisphere.purkinje_cells[0].parallel_fiber_weights.clone();

        let increased = final_weights_ltp
            .iter()
        ...
        let final_weights_ltd = hemisphere.purkinje_cells[0].parallel_fiber_weights.clone();

        let decreased = final_weights_ltd
            .iter()
    ...
    fn test_climbing_fiber_error_signal() {
        ...
        assert!(hemisphere
            .purkinje_cells
            .iter()
            .any(|pk| pk.climbing_fiber_active));
    }
    ...
    fn test_full_cerebellum() {
        ...
        for _ in 0..100 {
            cerebellum.update(1.0, &left_input, &right_input, &left_error, &right_error);
        }
