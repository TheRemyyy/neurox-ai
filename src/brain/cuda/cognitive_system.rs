...
use crate::brain::cuda::cognitive_kernels::*;
use crate::brain::cuda::context::CudaContext;
...
impl GpuCognitiveSystem {
    ...
    pub fn compute_attention(
        ...
        // Run kernel
        let config = crate::brain::cuda::KernelConfig::for_neurons(n_keys);
        self.spike_corr.launch(
            config,
            &query_gpu,
            &keys_gpu,
        ...
        // Apply softmax
        self.softmax
            .launch(config, &mut self.scores_buffer, n_keys as i32)?;

        // Download result
        let mut scores = vec![0.0; n_keys];
        self.device
            .dtoh_sync_copy_into(&self.scores_buffer.slice(0..n_keys), &mut scores)?;
