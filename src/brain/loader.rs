...
use crate::language::{IntentType, PartOfSpeech, SentenceTemplate, PragmaticRule};
...
impl BrainLoader {
    pub fn load_json_training(brain: &mut NeuromorphicBrain, file_path: &str) -> Result<(), Box<dyn Error>> {
        let dataset = JsonDataset::from_file(file_path)?;
        let stats = dataset.stats();
        println!("  Loaded: {} pairs ({} positive, {} negative)", 
        ...
        if let Some(markers) = &dataset.metacognition_markers {
            println!("  Loading Metacognition: {} reasoning, {} uncertainty markers", 
                markers.reasoning_markers.len(), markers.uncertainty_markers.len());
        ...
        println!("  Loading vocabulary: {} words with semantics", dataset.vocabulary.len());
        for vocab_word in &dataset.vocabulary {
            ...
            brain.language.ventral.embeddings.add_word(vocab_word.word.clone());
        }
        
        // === STEP 2: Load sentence templates into IFG ===
        println!("  Loading templates: {} sentence structures", dataset.sentence_templates.len());
        for template in &dataset.sentence_templates {
            let structure: Vec<PartOfSpeech> = template.structure.iter()
        ...
        println!("  Loading pragmatic rules: {} how-to-respond rules", dataset.pragmatic_rules.len());
        ...
        println!("  Loading intent rules: {} classification patterns", dataset.intent_rules.len());
        ...
        for (i, pair) in dataset.pairs.iter().enumerate() {
            ...
            brain.train_supervised_batch(
                &pair.input,
                pair.output.as_deref(),
                pair.reward
            );
            ...
            if let Some(ref output) = pair.output {
                ...
                if pair.reward > 0.0 {
                    ...
                    brain.ifg_planner.learned_responses.entry(intent_str.to_string())
                        .or_default()
                        .push(output.clone());
                    ...
                    for trigger in triggers {
                        ...
                        brain.ifg_planner.direct_memory.entry(key)
                            .or_default()
    ...
    pub fn load_text_training(brain: &mut NeuromorphicBrain, file_path: &str, window_size: usize) -> Result<(), Box<dyn Error>> {
        let dataset = TextDataset::from_file(file_path, window_size)?;
        let stats = dataset.stats();
        println!("  Načteno {} vět, {} slov, slovník {} slov", 
        ...
        for (batch_idx, chunk) in pairs.chunks(batch_size).enumerate() {
            let brain_pairs: Vec<(usize, usize)> = chunk.iter()
                .filter_map(|(c, ctx)| {
