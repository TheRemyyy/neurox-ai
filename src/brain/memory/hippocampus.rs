...
use crate::brain::neuron::LIFNeuron;
...
impl Hippocampus {
    ...
    pub fn new(input_dim: usize, scale: usize, sparsity: f32, buffer_capacity: usize) -> Self {
        ...
        let ca3_neurons = (0..ca3_size)
            .map(|i| LIFNeuron::new((dg_size + i) as u32))
            .collect();
        let ca1_neurons = (0..ca1_size)
            .map(|i| LIFNeuron::new((dg_size + ca3_size + i) as u32))
            .collect();
        ...
        for w in &mut ca3_recurrent_weights.weights {
            *w *= 0.1;
        }
    ...
    pub fn encode(&mut self, pattern: &[f32]) -> u32 {
        ...
        let ca3_output: Vec<f32> = ca3_input
            .iter()
            .map(|&x| (x * 2.0).tanh().max(0.0))
            .collect();
    ...
    pub fn recall(&mut self, cue: &[f32]) -> Vec<f32> {
        ...
        for _ in 0..iterations {
            ...
            let recurrent_input =
                self.project(&ca3_state, &self.ca3_recurrent_weights, self.ca3_size);

            // Update state (leaky integrator + nonlinearity)
            for i in 0..self.ca3_size {
                ca3_state[i] = (ca3_state[i] * 0.5 + recurrent_input[i] * 0.5)
                    .tanh()
                    .max(0.0);
            }
        }
...
mod tests {
    ...
    fn test_pattern_separation() {
        ...
        let dg1 = hc.apply_kwta(
            &hc.project(&pat1, &hc.ec_dg_weights, hc.dg_size),
            hc.dg_sparsity,
        );
        let dg2 = hc.apply_kwta(
            &hc.project(&pat2, &hc.ec_dg_weights, hc.dg_size),
            hc.dg_sparsity,
        );
